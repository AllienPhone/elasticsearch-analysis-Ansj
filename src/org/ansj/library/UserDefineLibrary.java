...
public class UserDefineLibrary {
	...
	public static void Init(Settings settings) {
		...
		logger.info(environment.configFile()+MyStaticValue.userDefinePath);
		try {
			...
			if (userDic.isFile()) {
				...
				logger.info("用户自定义词典加载完毕!");
			} else {

				System.err.println("用户自定义词典:" + MyStaticValue.userDefinePath
						+ ", 没有这个文件!");
			}
			logger.info("[Dict Loading] {},UserDict Time:{}",
					userDic.toString(), (System.currentTimeMillis() - start));
		} catch (Exception e) {
